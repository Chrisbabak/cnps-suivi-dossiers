// ---------------------------------------------------------------------------
// Écran de connexion (POC) : identifiant + mot de passe.
// Le compte désigne une personne ; son rôle vient du compte et son agence
// est retrouvée dans l'annuaire. La session survit au rechargement.
// ---------------------------------------------------------------------------
...
export default function Connexion({ onConnexion }) {
  ...
  const soumettre = (e) => {
    ...
    let agence = null
    if (compte.role === 'technicien') agence = agenceDeAgent(settings, compte.nom)
    if (compte.role === 'manager') agence = agenceDeManager(settings, compte.nom)
    if (compte.role !== 'admin' && !agence) {
      setErreur(
        `${compte.nom} n'est rattaché à aucune agence. Un admin doit l'ajouter à l'annuaire dans Paramètres.`,
      )
      return
    }

    ouvrirSession({ role: compte.role, agence, nom: compte.nom })
